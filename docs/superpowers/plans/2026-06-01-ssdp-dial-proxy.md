...
**Architecture:** A terminating TCP reverse proxy integrated into the existing single-threaded reactor. The reactor gains read/write interest control (for non-blocking `connect()` completion, write backpressure, and read-side flow control). `DialProxy` (owned by `SsdpReflector`, gated by `dial = true`) rewrites the DIAL `LOCATION` in proxied SSDP responses to point at itself, stands up an ephemeral TCP listener per discovered device endpoint, and for each accepted client connection opens an upstream connection bound to `target_if`'s address (so the device sees an its-subnet source). It forwards HTTP, rewriting only four headers (`LOCATION`, `Application-URL`, response `Location`, request `Host`) by authority substitution — bodies stream through verbatim (Content-Length or chunked). State is bounded (drop-new caps + role-based idle eviction + flow-controlled buffers), all RAII, all reactor-driven, no threads. Grounded in a packet capture of a real LG webOS TV and the DIAL v1.6.4 spec. Full design: `docs/superpowers/specs/2026-06-01-ssdp-dial-proxy-design.md`.
...
4. **`dial_proxy`** — the `DialProxy` orchestrator (listeners, connection pump, flow control, eviction).
...
- `src/reflector/dial_proxy.{h,cpp}` — `DialProxy`, `Endpoint`/`Connection`/`SendBuffer`, the connection pump, flow control, eviction `Timer`. *(Commit 4)*

**Modified source files**
- `src/reflector/dispatcher.h`, `src/reflector/event_loop_dispatcher.{h,cpp}` — `OnWritableCallback`, 3-arg `Register`, `SetWriteInterest`, `SetReadInterest`, `FdCallbacks`. *(Commit 1)*
...
- New: `tests/fake_dispatcher_test.cpp` *(C1)*, `tests/ip_endpoint_test.cpp` + `tests/tcp_socket_test.cpp` *(C2)*, `tests/http_message_test.cpp` *(C3)*, `tests/dial_proxy_test.cpp` *(C4)*.
...
- Mocks: `tests/mocks/fake_dispatcher.h` gains `FireWritable`/`SetReadInterest`/`SetWriteInterest`/`IsReadArmed`/`IsWriteArmed` *(C1)* and `WatchedFds()` *(C4)*.
...
This commit gives the readability-only reactor two new controls — a write callback with dynamic write interest, and dynamic read interest for backpressure — landed and tested in isolation, exactly like the step-2 timer commit, before any DIAL code. After this commit, every existing consumer is unaffected (read interest starts armed; the 2-arg `Register` still compiles), and the proxy (Commit 4) can apply level-triggered-safe flow control via `SetReadInterest`.
