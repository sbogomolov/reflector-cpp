...
> **Draft — predates decisions settled during Commits 1–2; rewrite this section when you execute it.** Reconcile against the **interface-contract block** above and the design spec. What changed since this was written:
> - **Backpressure is drop-and-close, not read-interest flow control.** `SendBuffer` (shipped in Commit 2) is bounded at `MAX_SEND_BUFFER` (64 KB); a `Send` past the cap aborts the connection. No `HIGH_WATER`/`LOW_WATER`, no `SetReadInterest`-based pause — read stays always-armed. (Task 4.1 is obsolete — `SendBuffer` already exists — and the flow-control steps go away.)
> - **B2 boundary:** the `Connection`/`Endpoint` is `NoMove`, the stable callback target; it holds the `Registration`s and forwards `TcpSocket::WantsWrite()` via one `Sync()`. `TcpSocket` is dispatcher-inert.
> - **As-built `TcpSocket` API:** `optional<TcpSocket>` factories; `Connect(IpEndpoint dst, IpEndpoint bind, unsigned ifindex)`; `FinishConnect()` (not `SoError()`); `LocalEndpoint()` (not `LocalPort()`).
> - **Bind address + ifindex come from the source/target `LinkSocket&`** the proxy already holds (`SourceAddress(family)` + `InterfaceIndex()`), read at establish time — not from `IpAddress`/iface-name members (the deferred "Interface class" question lives here).
> - **REST-listener port stability** (vs the discovery listener's ephemeral port) is still to be decided here.

This commit lands `src/reflector/dial_proxy.{h,cpp}`: the `Endpoint`/`Connection` state, the two listener caps + `MAX_CONNECTIONS` drop-new, `EnsureListener`/`EnsureDiscoveryListener`/`EnsureRestListener`, the non-blocking connection pump, the connect/idle eviction `Timer`, and drop-and-close backpressure over the bounded `SendBuffer` from Commit 2.
