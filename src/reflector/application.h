...
// Owns and wires together the capture sockets, the dispatcher and packet dispatcher, and the
...
// wiring testable: the capture-socket factory can be overridden to inject sockets wrapping fake
...
public:
    ...
    // Builds one reflector per WolConfig, sharing a single capture socket across configs on
...
private:
    ...
    // Declaration order is the teardown order in reverse: reflectors drop their packet-dispatcher
    // registrations first, the packet dispatcher then drops its per-socket dispatcher
    // registrations, the dispatcher tears down its event queue, and only then the capture
